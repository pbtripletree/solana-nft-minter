...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    if (currentDate > dropDate) setIsDropped(true);
    ...
    for (var i = 0; i < itemsRedeemed; i++) {
      const data = await fetch(nftData.items[i].link).then((response) =>
        response.json()
      );
      mintedItems.push(data);
    }
      ...
      <div className="machine-container">
        ...
        candyMachine.state.itemsAvailable ? (
          ...
          <button
            ...
            onClick={mintToken}
            disabled={isMinting || !isDropped}
          >
            {isDropped ? "Mint NFT" : "Coming soon!"}
          </button>
        )}
